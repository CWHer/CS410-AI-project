    ...
    def __enhanceData(self,
        ...
        for state, reward, action, prev_action, next_state, done in zip(
            ...
            for i in [0, 2]:
                ...
                new_action = Actions.rot90(Actions(action), i)
                new_prev_action = Actions.rot90(Actions(prev_action), i)
                new_actions = ActionsFilter.genActions(new_prev_action)
                # NOTE: calculate relative index
                enhanced_data[i & 1].append(
                    (new_state, reward,
                     new_actions.index(new_action.value), new_next_state, done))
                ...
                new_action = Actions.fliplr(new_action)
                new_prev_action = Actions.fliplr(new_prev_action)
                new_actions = ActionsFilter.genActions(new_prev_action)
                # NOTE: calculate relative index
                enhanced_data[i & 1].append(
                    (new_state, reward,
