...
class SnakeEatBeans(GridGame, GridObservation, DictObservation):
    def __init__(self, conf):
        ...
        if self.n_player * self.init_len > self.board_height * self.board_width:
            raise Exception("玩家数量过多：%d，超出board范围：%d，%d" % (
    ...
    def get_dict_observation(self, current_state, player_id, info_before):
        ...
        key_info['last_direction'] = info_before.get(
            'directions') if isinstance(info_before, dict) else None
    ...
    def init_state(self):
        for i in range(self.n_player):
            s = Snake(i + 2, self.board_width,
                      self.board_height, self.init_len)
            s_len = 1
    ...
    def update_state(self):
        next_state = [
            [[0] * self.cell_dim for _ in range(self.board_width)] for _ in range(self.board_height)]
        for i in range(self.n_player):
            snake = self.players[i]
    ...
    def generate_beans(self):
        all_valid_positions = set(itertools.product(
            range(0, self.board_height), range(0, self.board_width)))
        all_valid_positions = all_valid_positions - \
            set(map(tuple, self.beans_position))
        for positions in self.snakes_position.values():
            all_valid_positions = all_valid_positions - \
                set(map(tuple, positions))
    ...
    def get_all_observes(self, before_info=''):
        self.all_observes = []
        for i in range(self.n_player):
            each_obs = self.get_dict_observation(
                self.current_state, i+2, before_info)
        ...
        if not not_valid:
            ...
            for i in range(self.n_player):
                ...
                if self.be_eaten(snake.headPos):  # @yanxue
                    ...
                    snake.pop()
            snake_position = [
                [-1] * self.board_width for _ in range(self.board_height)]
            ...
            for i in range(self.n_player):
                ...
                if re_generatelist[i] == 1:
                    if eat_snakes[i] == 1:
                        snake.snake_reward = self.init_len - \
                            len(snake.segments) + 1
                    else:
                        snake.snake_reward = self.init_len - \
                            len(snake.segments)
                    snake.segments = []
            ...
            for i in range(self.n_player):
                ...
                self.won[i] = s.score
            info_after = {}
            info_after["snakes_position"] = [list(v) for k, v in sorted(
