...
class GridGame(Game):
    def __init__(self, conf, colors=None, unit_size=UNIT, fix=FIX):
        ...
        self.grid = GridGame.init_board(
            self.board_width, self.board_height, unit_size)
        ...
        for i in range(self.board_height):
            for j in range(self.board_width):
                ...
                for k in range(self.cell_dim):
                    grid_map[i][j] = grid_map[i][j] * \
                        self.cell_range[k] + current_state[i][j][k]
        return grid_map
    ...
    def init_board(width, height, grid_unit, color=(250, 235, 215)):
        im = Image.new(mode="RGB", size=(
            width * grid_unit, height * grid_unit), color=color)
        draw = ImageDraw.Draw(im)
        for x in range(0, width):
            draw.line(((x * grid_unit, 0), (x * grid_unit,
                      height * grid_unit)), fill=(105, 105, 105))
        for y in range(0, height):
            draw.line(((0, y * grid_unit), (width * grid_unit,
                      y * grid_unit)), fill=(105, 105, 105))
        return im
    ...
    def _render_board(state, board, colors, unit, fix, extra_info=None):
        ...
        for x, row in zip(count(0), state):
            for y, state in zip(count(0), row):
                if state == 0:
                    continue
                draw.rectangle(build_rectangle(y, x, unit, fix), fill=tuple(
                    colors[state]), outline=(192, 192, 192))
        return im
