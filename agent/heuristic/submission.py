...
class Board:
    def __init__(self, board_height, board_width, snakes, beans_positions, teams):
        ...
        self.board = np.zeros((board_height, board_width),
                              dtype=int) + self.blank_sign
        self.open = dict()
        for key, snake in self.snakes.items():
            # state 0 open list, heads, ready to spread
            self.open[key] = [snake.head]
    ...
    def step(self):  # delay: prevent rear-end collision
        ...
        for key, snake in self.snakes.items():
            if snake.len >= self.state:
                self.board[snake.pos[-self.state][0]][snake.pos[-self.state]
                                                      [1]] = self.blank_sign  # drop tail
        # value: e.g. [[8, 3], [6, 3], [7, 4]]
            ...
            for x, y in value:
                ...
                for x_, y_ in [((x + 1) % self.height, y),  # down
                    ...
                    if sign == self.blank_sign:  # grid in initial state
                        ...
                        if self.snakes[idx].len > self.snakes[key].len:
                            print(
                            ...
                            new_open[key].append([x_, y_])
                        # controversial claim
                        elif self.snakes[idx].len == self.snakes[key].len:
                            print(
                            ...
                            # controversy, no snake claim this grid!!
                            self.snakes[idx].claimed_count -= 1
                            new_open[key].append([x_, y_])
                        ...
                        if controversy['length'] > self.snakes[key].len:
                            ...
                            new_open[key].append([x_, y_])
                        # controversial claim
                        elif controversy['length'] == self.snakes[key].len:
                            print(
    ...
    def claim2action(self, claim_position, snake_idx, step_count, output_type):
        ...
        while step_count > 1:
            ...
            for x_, y_ in [((x + 1) % self.height, y),  # down
                ...
                state = sign // self.snakes_count if sign > 0 else - \
                    (sign // self.snakes_count)
                indexes = [sign % self.snakes_count] if sign >= 0 else self.controversy[(
                    x_, y_)]['indexes']
...
def state2claims(state_array, max_state, priority):
    ...
    horiz = [list(state_array[_]).count(max_state)
             for _ in range(beanCount)]  # [5, 5, 4, 4, 5]
    vert = [list(state_array[:, _]).count(max_state)
            for _ in range(snakeCount)]  # [4, 4, 4, 3, 4, 4]
    claim_order = []
    for b in range(beanCount):
        for s in range(snakeCount):
            if state_array[b][s] < max_state:
                claim_order.append(
                    [b, s, state_array[b][s], -horiz[b] - vert[s]])
...
def my_controller(observation_list, action_space_list, is_act_continuous):
    with HiddenPrints():
        ...
        # 0, 1, 2, 3, 4, 5
        ctrl_agent_index = obs['controlled_snake_index'] - 2
        # last_directions = obs['last_direction']  # ['up', 'left', 'down', 'left', 'left', 'left']
        # e.g.[[7, 15], [4, 14], [5, 12], [4, 12], [5, 7]]
        ...
        init_board = Board(board_height, board_width,
                           snakes, beans_positions, teams)
        bd = copy.deepcopy(init_board)

        with HiddenPrints():
            # loop until all values in open are empty list
            while not all(_ == [] for _ in bd.open.values()):
                bd.step()
        print(bd.board)
        ...
        defense_threshold = board_height * board_width * \
            teams_count / (teams_count + 1) / snakes_count
        ...
        state_array = np.zeros(
            (len(beans_positions), len(snakes)), dtype=int) + max_state
        for i, (x, y) in enumerate(beans_positions):
            ...
            # [2, 3, 4, 5]
            elif sign < 0 and sign % snakes_count in [_ for _ in range(snakes_count) if _ > 0]:
        ...
        claim_list_byState = state2claims(
            state_array.copy(), max_state, 'state')
        claim_list_byCover = state2claims(
            state_array.copy(), max_state, 'cover')
        print('claim_list_byState: ', len(
            claim_list_byState), claim_list_byState)
        print('claim_list_byCover: ', len(
            claim_list_byCover), claim_list_byCover)
        claim_list = claim_list_byState if len(claim_list_byState) >= len(
            claim_list_byCover) else claim_list_byCover
        ...
        for c in claim_list:
            if ctrl_agent_index == c[1]:  # the controlled agent claim a bean
                print('the controlled agent {} claim a bean {} within {} step(s)'.format(
                    c[1], c[0], c[2]))
            ...
            free_team_snakes_indexes = list(np.array(free_team_snakes_indexes)[
                                            check_list])  # [idx1, idx2]
            # [[pos1, pos2, pos3], [pos1, pos2, pos3]
            safe_positions_list = [_ for _ in safe_positions_list if _]
            ...
            for pos_comb in itertools.product(
                ...
                bd_next = Board(board_height, board_width,
                                snakes_next, beans_positions, teams)
                with HiddenPrints():
                    while not all(
                ...
                for i, idx in enumerate(team_indexes):
            ...
            print('max_claimed_counts_sum: ', max_claimed_counts_sum,
                  'best_pos_comb: ', best_pos_comb)
            ...
            if best_pos_comb:
                for i, idx in enumerate(free_team_snakes_indexes):
                    if ctrl_agent_index == idx:
                        ...
                        print('the controlled agent {} make a safe move'.format(
                            idx), action[0])
                        print(
                            '*********************************** safe move ******************************************')
                        return action
            ...
            action[0][direction] = 1
            print('the controlled agent {} play a random action and is dying'.format(
                ctrl_agent_index), action[0])
            print(
                '*********************************** random move ******************************************')
            return action
